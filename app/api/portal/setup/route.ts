...
export const POST = apiHandler(async function POST(req: Request) {
  const limit = await limitByIp(req.headers, "setup", 10, 15 * 60 * 1000);
  ...
  const user = await setPasswordFromLink("client", parsed.data.token, parsed.data.password);
  await createSession({ sub: user.id, name: user.name, email: user.email, role: "CLIENT", clinicId: user.clinicId, sessionVersion: user.sessionVersion });
